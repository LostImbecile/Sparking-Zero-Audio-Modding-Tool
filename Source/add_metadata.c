...
int add_metadata(const char* input_file) {
    ...
    strcpy(awb_path, input_file);

    // Acbs would show all their cues, has to be the awb file
    replace_extension(awb_path, "awb");
    ...
    snprintf(metadata_batch_path, sizeof(metadata_batch_path), "%s\\add_metadata.bat", folder_path);
    ...
    if ((dir = opendir(folder_path)) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            ...
            if ((is_bgm || strstr(filename, "_streaming") != NULL) &&
                ...
                snprintf(wav_file_path, sizeof(wav_file_path), "%s\\%s", folder_path, filename);
