	...
	for (int i = 0; i < bgm_entry_count; i++) {
		if (strcasecmp(filename, bgm_entries[i].cueName) == 0 ||
			...
			if (bgm_entries[i].index == 82 || bgm_entries[i].index == 83 ||
			        ...
			        bgm_entries[i].index == 120 ||
			        bgm_entries[i].index == 121 ||
			        bgm_entries[i].index == 133 ||
			        bgm_entries[i].index == 134) {
			...
			for (int j = 0; j < *injection_count; j++) {
				if (injections[j].index == bgm_entries[i].index ||
				        is_index_in_pair(injections[j].index, bgm_entries[i].index)) {
					printf("Warning: Ignoring file '%s' as index %d (or its hca pair) has already been processed\n",
					       extract_name_from_path( filepath), bgm_entries[i].index);
					return false;
				}
			}
...
bool is_index_in_pair(int index1, int index2) {
	for (int i = 0; i < hca_pair_count; i++) {
		if ((hca_pairs[i].index1 == index1 && hca_pairs[i].index2 == index2) ||
		        (hca_pairs[i].index1 == index2 && hca_pairs[i].index2 == index1)) {
			return true;
		}
	}
	return false;
...
bool process_hca_entry(const char* filepath, const char* dirpath,
	...
	for (int i = 0; i < bgm_entry_count; i++) {
		if (bgm_entries[i].index == index) {
			bgm_entry = &bgm_entries[i];
			break;
		}
	}
...
void process_paired_hca_entries(const char* filepath, const char* dirpath,
	...
	for (int i = 0; i < hca_pair_count; i++) {
		int paired_index = -1;
		if (hca_pairs[i].index1 == main_index) {
			paired_index = hca_pairs[i].index2;
		} else if (hca_pairs[i].index2 == main_index) {
			paired_index = hca_pairs[i].index1;
		}
...
const char* get_container_file(const char* target_file,
	...
	for (int i = 0; i < acb_mapping_count; i++) {
		if (strcasecmp(acb_mappings[i].awbName,
			...
			snprintf(container_path, sizeof(container_path), "%s\\%s", dir_name,
			         replace_extension(acb_mappings[i].acbName, "uasset"));
			if (stat(container_path, &(struct stat) {
			0
		}) == 0) {
				return container_path;
			...
			snprintf(container_path, sizeof(container_path), "%s\\%s", dir_name,
			         replace_extension(acb_mappings[i].acbName, "acb"));
			if (stat(container_path, &(struct stat) {
			0
		}) == 0) {
				return container_path;
    ...
    if (injection_count > 0) {
        ...
        for (int i = 0; i < injection_count; i++) {
            ...
            if (container_index == -1) {
                ...
                grouped_injections[container_count] = malloc(sizeof(InjectionInfo) * (injection_count + 1));
